...
class MainEventController: UIViewController {
    ...
    func showAlert() {
        ...
        let doneAction = UIAlertAction(title: "Done", style: .default){
            ...
            self.eventNameArray.append((alert.textFields?.first?.text)!)
            self.webCodeArray.append((alert.textFields?.last?.text)!)
            self.eventImageArray.append(UIImage(named: "camera"))
...
extension MainEventController: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.webCodeArray.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setData(webCode: self.webCodeArray.reversed()[indexPath.row],eventName: self.eventNameArray.reversed()[indexPath.row], image: self.eventImageArray.reversed()[indexPath.row]!)
        
        return cell
    }
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print(webCodeArray.reversed()[indexPath.row])
        print(indexPath.row)
        //TODO: add library access and create separate album for every web-code
        //TODO: create next viewController with the access to camera and add image processing
        
        PHPhotoLibrary.requestAuthorization { status in
            switch status {
            case .authorized:
                // Permission Granted
                createPhotoLibraryAlbum(name: self.webCodeArray.reversed()[indexPath.row])
            case .denied:
            // Permission Denied
                print("User denied")
            default:
